...
The recommended reference UI is a compact square status indicator beside the status label, with tooltip/help text built from `ModStatusDisplay`. Mod writers can render status however they like, but copying `examples/fabric-embedded-reference` should produce the same square status box across mods.
